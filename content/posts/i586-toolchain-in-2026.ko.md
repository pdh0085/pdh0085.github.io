...
Quark X1000의 코어는 Lakemont다. i586이고, MMX/SSE가 없고, SMP가 없고,
...
포트는 수학 함수를 x87 인라인 asm으로 구현하고, 소프트플로트용 i386 변종이
없다. x87 레지스터를 요구하는 asm을 x87이 꺼진 컴파일러에 먹였으니
`impossible constraint`가 나온 것이다.

그리고 하드웨어에는 x87이 실제로 있다. 기존 Yocto 이미지가 i586 x87
유저스페이스를 잘 돌리고 있었고, 그 커널의 `MATH_EMULATION`도 꺼져 있었다.
오래된 ISA를 타깃할 때 문제는 명령어가 없다는 데서 오는 게 아니라,
**툴체인의 기본값이 내 하드웨어와 다르게 가정한다**는 데서 온다.
...
확인했다. x87만 켜지고 MMX/SSE는 계속 꺼져 있어야 하는데, 실제로 그랬다.

한 가지 더. soft-float에서 hard-float로 가는 건 부동소수점 ABI 변경이다.
...
- 오래된 타깃에서 싸움의 상대는 없는 명령어가 아니라 틀린 기본값이다.
  `-march=lakemont`가 soft-float를 기본으로 켤 줄은 켜보기 전엔 모른다.
- 컴파일러의 가정은 하드웨어로 반증할 수 있다. 잘 돌던 기존 펌웨어, 그러니까
  x87 유저스페이스에 `MATH_EMULATION=n`인 커널이 그 증거였다.
- 부동소수점 플래그는 ABI다. 바꿨으면 전부 다시 짓는다.
- 크로스 빌드의 검증은 플래그가 아니라 산출물에 한다. 285개를 세는 시간이
  보드에서 SIGILL을 쫓는 시간보다 훨씬 싸다.

이 단계에서 얻은 것은 부팅이 아니라 정확한 바닥이다. 모든 바이너리가 이
