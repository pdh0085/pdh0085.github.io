...
IPQ8072, based on OpenWrt 25.12) isn't the feature list — it's **the
fraction of incidents that end without anyone driving anywhere.**
...
And let me say it up front: the table **still has gap rows.** We wrote them
down instead of erasing them. The last section of this post is exactly those
rows.
...
## Case 1 — The Zombie Modem: 104 Seconds, Unattended
...
reserved address, and you get precisely that state: the line is up, nothing
arrives. Result: three failures at 45-second intervals → hardware reset,
**93 seconds from the first failure.**

**The zombie.** The interface exists but nobody answers. We created a logical
USB disconnect with every power rail still healthy. Result: **detected at 41
seconds → reset → re-enumerated 32 seconds later → fully back to normal 104
seconds after the disconnect.** No human involved.
...
The fix was ordering: first **wait for the module to disappear from the
bus**, then confirm its return (measured: gone at 1 second after reset, back
at 30). In the re-drill the ladder stopped at the first rung, as designed.
...
We drilled it with the grace period shortened. Bad WAN config applied →
**restored at 124 seconds → line back to normal at about 130 seconds**, no
human intervention. That 130 seconds is a drill value with the grace
shortened; **the shipping default grace is 10 minutes.**

We drilled the false-positive defenses too. A change made while the line is
already dead is accepted, not reverted — otherwise the mechanism would fight
the very repair work being attempted. And a revert never happens twice.

This row also carries a **gap row right next to it.** The device's only
health signal is the WAN, so a firewall mistake that leaves the line alive
but blocks only the path to the management server is not caught. Changes
...
## Case 4 — The Reset Button: The Row Where Our Document Was Wrong
...
**two** reset paths, both firing on a single button press, because the
system runs both kinds of button handler.
...
We drilled it with three real presses: **1 second → WPS pairing. 6 seconds →
nothing at all** (a press that would have wiped the box under the old
threshold). **11 seconds → factory reset, back up in about 30 seconds.** And
we read the reset log back *after* the reboot — this reset is auditable
after the fact.
...
- **Remote log collection.** The management server pushes four parameters
  ...
  And the bundle deliberately excludes one specific log file, because at
...
- **Delivering improved defaults to fielded units.** How do you get a better
  ...
  management server" — this product ships with no management server address
  configured, so that answer would freeze every retail unit at factory
  defaults forever. Instead we built a numbered migration scheme and
  induced-tested that the failover-timer improvement from an earlier post in
  this series (51.30 s → 9.58 s) actually reaches a unit that never had it.
  After a settings-preserving upgrade the value converged, and on the second
  reboot nothing happened at all — it's idempotent.

The most important thing about migrations is not the mechanism but the
boundary: a migration fixes **only what is measurably wrong** and **never
touches a value the customer chose.** And there is no mechanism enforcing
that boundary — it is held by discipline. The document says exactly that,
too.
...
- **The hardware watchdog** — we verified it is shipped and running (30-second
  ...
  a safety net.
- **Remote management over the 5G path** — every management-session
  measurement was taken over Ethernet. It should work over 5G by design;
  it has never been demonstrated. Grade: **inference.**
- **A bad config that blocks only the management server while the line stays
  up** — the gap row from Case 2. Grade: **gap.**
...
- "It works" is not an answer. The grade of evidence behind the claim is the
  answer, and four grades are enough: measured, armed, inference, gap.
- There is only one way to upgrade a row: actually break the thing. And a
  drill's real payoff is rarely the number — it's the defect hiding inside
  something that "was working."
- The most valuable rows in the table are the gap rows. Writing down what
  you can't do first is what makes the rest of your numbers believable.
- Put a ceiling on your automation. A reboot erases its own logs, so after
  three attempts we stop and preserve the evidence instead.
- The boundary of automatic fixes is discipline, not technology. Fix only
  what is measurably wrong, never touch what the customer chose — and write
  that discipline into the document.
