...
There's a footnote to attach, though. This line rate comes from software
forwarding, and it pays for itself with roughly two of the four cores.
Getting those two cores back via hardware offloading is still an open
task.
...
This difference doesn't show up as speed. It shows up as headroom. On a
box that already spends two cores on forwarding, whether the tunnel eats
1.5 cores or 2.5 decides whether anything else can still run. On a spec
sheet the two rows look interchangeable; in practice, one of them
consumes the box.
...
So we position OpenVPN by reachability, not speed. Its TCP transport is
30% slower than UDP, but on networks that block ESP and non-web UDP
outright, it is the only tunnel left standing.

### Our First IPsec "Success" Was Plaintext
...
### The Numbers You Must Not Quote
...
existing connections; when the flush list is empty, it flushes nothing.
So established flows keep the dead line's mark forever. The route
changes; the traffic doesn't follow.
...
- "It's connected" and "the traffic took that path" are different claims.
  If no counter moved, nothing happened, and a green status light is not
  evidence.
- A number only becomes a value once its conditions are attached: 9.58 s
  ...
  "1 meter apart." A number without conditions is an inflated number. For
  the same reason, saying first which numbers must not be quoted is what
  buys trust; if you can explain why you discard 840/889 and publish 529,
  the rest of your numbers become believable.
- Narrow your own findings yourself. The flush setting rescued failover
  but does not rescue TCP sessions, and omitting that one line would turn
  a finding into an exaggeration.
