...
error-free, while doing nothing at all. And they were broken for different
reasons — one at a process boundary, one inside a misfiring abstraction.
...
`SAVE_CONFIG=1` — which means the *absence* of `$UPGRADE_BACKUP` is the
wipe signal. I switched the condition to that and verified both directions
on the serial console: with `-n`, the overlay-removal log line appears and
the planted markers (an APN, the hostname) are gone; with a normal upgrade,
they survive.

One-line lesson: shell's `${VAR:-default}` is convenient, but it cannot
tell "the variable is unset" apart from "the variable holds the default."
In code that crosses a process boundary, that's dangerous.
...
file sitting on FAT, so none of the drivers — mtd (there is no `/proc/mtd`
at all), ubi, partname — can match it.
...
First fix — which then rots. I added a fallback: "if the volume can't be
...
- `volume_find()` succeeds — which means the fallback never fires
...
- Recovery features are never exercised by normal use. The only option is
  to actually press them, every release. Neither of these bugs would ever
  have surfaced on the happy path.
- "Failed, but returned 0" is the worst kind of failure. The harder a
  feature's outcome is to observe — resets, upgrades — the more its exit
  code and its post-state both need checking.
- An abstraction can coexist with a safety net only when it admits what it
  doesn't know. `rootdisk` answered confidently without understanding our
  layout, and that confidence killed the fallback. Of course, half the
  cause is our own odd loop-ext4-on-FAT arrangement — if you choose to
  live outside the world an abstraction assumes, doubting its answers
  becomes your job too.
