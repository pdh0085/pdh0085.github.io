...
This one is from a while back, so the exact logs are long gone; the commands and
...
outside. WAN was up, the LTE link was fine, remote access worked. The box
itself was clearly healthy, so the finger naturally pointed at the forwarding
rules.
...
fine, web UI and SSH both. Camera healthy, router healthy, rules apparently
correct, external access dead.
...
Taiwan office, which I drove over remote desktop from my desk in Korea, so I
could hammer the router's public address and watch the interface packet
...
kept climbing while RX stayed flat.**

That one observation flipped the whole picture. The port forwarding was
working; the router had been translating and delivering packets to the camera
the entire time. The camera was receiving *something* and refusing to answer.
My working theory became: whatever the camera is receiving, it's arriving in
a state the camera's TCP stack won't accept, so it silently drops it.

## What tcpdump Showed

I installed tcpdump on the router and captured the forwarded traffic. Small
packets (the TCP handshake and friends) looked clean. But the large segments
...
In hindsight, the red herring explains itself too. The router's own web UI
survived because its exchanges happened to stay small enough to slip under
...
Everything worked, instantly. Web UI, SSH, video.

I won't pretend I can reconstruct the exact mechanism from memory (MSS
following the interface MTU down for forwarded connections would be the usual
suspect), but the observable result was unambiguous: cap the WAN at 1400 and
nothing on the link gets bitten.
...
This is the part that never makes it into tidy postmortems: I never got a
...
vendor, so we worked the issue together: we characterized it, they fixed the
firmware, done. This time the modem came from a tier-1 vendor, and
realistically a single field case from one CPE maker was not going to open a
firmware investigation there.
...
added a manual MTU override, so if the field says the path can't carry what
the modem claims, an operator can pin the WAN MTU themselves, per deployment,
without waiting for anyone's firmware.

One field incident bought us a permanent knob, and that knob has paid for
itself since.

## Takeaways

1. "Small packets work, large packets die" is the MTU smell. Partial
   connectivity (a reachable UI, hanging transfers, SSH that connects and
   then stalls) should make you think packet size before configuration.
2. Interface counters are a free diagnostic. TX climbing while RX stayed
   flat told me more in five minutes than hours of rule-staring did.
...
4. On cellular, don't assume the path delivers 1500 bytes, and don't assume
   anything will tell you when it doesn't. Design as if silent large-packet
   loss is a thing, because it is.
5. You won't always get root cause, and you should ship the mitigation
   anyway. Between a tier-1 vendor's queue and a customer's deadline, a
   well-designed override in your own firmware wins. Make the fix a product
   feature, not a war story.
