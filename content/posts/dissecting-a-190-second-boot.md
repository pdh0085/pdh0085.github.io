...
Let me say up front that this is not a "we cut boot from three minutes to
one" success story. What actually happened is that I closed one large hole
and knocked down one inherited hypothesis.
...
No `ACTION` check, no `DEVTYPE` check, so it runs on every USB uevent. And
one modem generates ten of those: one device plus nine interfaces. Each run
re-parses the 55 KB, 3,359-line `/etc/usb-mode.json` from scratch, which
...
The absurd part is that every modem we ship enumerates as a modem from the
...
takes about 7 seconds (a property of the module itself, not an ordering
problem), and since S99 runs at around 155 seconds, the modem didn't appear
until about 162 seconds. The device that needs the longest lead time was
being powered last.
...
So I moved the script to START=11. The modem now appears on USB at **about
50 seconds instead of about 162**, and Wi-Fi is essentially unaffected
...
conditions. It doesn't mean causation. I carried that belief around for
weeks, and there was only one way to test it: deliberately construct the
worst case and try to reproduce.
...
core, or 6–13 seconds when it fails because the bus isn't up yet. A
whitelist filter plus a no-bus guard took that from **23 forks to 0**. The
full story is in that earlier post.
...
the total unchanged? The rc.common profile shows the remaining chunks
plainly: S95frr about 30 seconds, S10boot about 16, S80ucitrack about 12,
S19firewall about 11, plus a late-boot stretch where things like sysntpd,
mwan3, dnsmasq, and firewall get invoked over and over. I have not
investigated these yet. I have the numbers and not the causes. That's the
list for next time.
...
- Decisions about start order (START=) belong to experiments, not comments.
  If you want to disprove a claim, deliberately construct the worst case for
  it and try to reproduce the failure.
- A design that avoids contention by starting late is usually a sign that
  nobody understood the cause. Once you do, the ordering becomes free.
