...
The core inside the Quark X1000 is Lakemont: i586, **no MMX, no SSE**, no
SMP — and it **does have an x87 FPU**. Those four facts dictate every
toolchain decision in this post.
...
Pause for a second: why? A missing instruction? No.
...
checked `gcc -Q --help=target`: **x87 on, MMX and SSE still off.**

One more thing. Going from soft-float to hard-float is a floating-point
**ABI change**. Objects built under the old assumption must not mix with the
new ones, so I deleted `build_dir/{toolchain,target}-i386_lakemont*` and
staging, and rebuilt from scratch.

## Verification: Don't Trust — Count

The flag was fixed and the build went through. That is not the place to
stop. "I passed the flag, so it must be fine" is not verification.
...
What this stage bought was not a boot — it was **a correct floor**: proof
that every binary uses only what this CPU actually has, and nothing it
doesn't. The next post in this series is about the kernel refusing to come
up on top of that floor.
