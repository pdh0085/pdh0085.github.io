...
`$GPSLOCK` is the modem's GNSS lock setting, and 3 means **fully locked**.
The board didn't have zero satellites. It had an engine it was forbidden to
start.
...
One command. A little while later, the "broken" board was tracking **22–25
satellites** — HDOP 1.99, horizontal accuracy ±9.9 m, the same quality as
the healthy bench unit. After a reboot it acquired a fix again entirely on
its own. The first five minutes of 4 satellites and HDOP 52? Just a cold
start.

### Why That Row Was Missing

This question matters more than the incident itself.

`$GPSLOCK` lives in the modem's **internal NV memory**. It does not appear
in anything we had compared — not the config files, not the antenna port
...
Here's the part that stings. The decisive clue had been in our hands from
the beginning.
...
evidence as known-normal behavior — and as a result, we had judged a
perfectly good board defective and were planning to start pulling antennas.
...
It wasn't our code — that much was certain. There isn't a single line in
our source tree that sets a GNSS lock. Which means the modem did it to
itself, and indeed this modem has a separate setting (`$GNSSDISACFG`) that
disables GNSS when certain host events occur. And that setting's **factory
default allows the lock.**

Which means:

- The lock can **come back at any time.**
- Since it's the factory default, **every unit in the field is exposed to
  the same condition.**

Fixing one unit and preventing recurrence are different jobs. We adopted
`$GNSSDISACFG=0` — no host event may disable GNSS — as the **shipping
default**. The cost is power consumption, and our positioning daemon already
holds a continuous session anyway, so most of that power saving had been
forfeited long ago.
...
- Check once at boot; after that, check periodically **only while there is
  no fix.**
- If the lock is set, clear it — and normalize the lock-permission setting
  at the same time.
...
Both are about how you decide "is GNSS healthy" — and neither would ever
have been caught by reasoning alone.
...
clock.** So computing the age gives you a negative number. Our first rule
was "negative = clock broken = untrusted" — and suddenly **every healthy
unit read as sick.** When a clock is actually wrong, it's off by years, not
seconds. A tolerance of about 60 seconds settled it.
...
unanswered: we still don't know **which host event** set the lock in the
first place. Today's mitigation is "clear it when it happens," not "make it
never happen." A follow-up inquiry is filed with the vendor; if an answer
comes, the polling check can become event-driven.

## What You Keep Is Habits, Not Conclusions

What this case taught us:

- **"Everything is the same" is true only if the table is complete.** A row
  the table doesn't have can't even be different.
- **Internal documentation can forge evidence.** One wrong line becomes
  "it's always been like that" and reclassifies the decisive clue as
  normal. Judgment criteria written in docs need to be re-measured against
  real hardware periodically.
- **Fixing one unit and preventing recurrence are different jobs.** Once
  you find root cause, the next question is always: "so why was this set,
  and can it be set again?"
- **A self-healing feature's health criteria must be measured on real
  hardware.** Whether a value keeps updating or freezes is written nowhere
  in the code.
