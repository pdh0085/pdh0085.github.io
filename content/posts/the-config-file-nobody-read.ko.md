...
세 단계로 쪼개서 봤다. UI에서 저장하고, 파일 내용을 확인하고, 마지막으로
실제 running-config를 확인한다. 1단계와 2단계는 성공했고, 3단계만 비어
있었다. 저장 성공은 도달을 증명하지 않는다.
...
설정(integrated-config) 모드로 전환한다는 것이다.
...
그리고 이 보드는 그 파일을 출하 시부터 갖고 있었다. 페이지는 몇 년째
...
- 설치 전에 `vtysh -C`로 드라이런 파싱을 한다. 문법 오류 하나가 다음
  재시작에서 라우팅 스택 전체를 내려버리기 때문이다. 거부되면 이전 설정이
  계속 돌고, 페이지가 그 사실을 알린다.
- 종료 코드를 계약으로 쓴다. `0` = 새 설정 설치됨(재시작), `2` = 변경
...
- 소유권을 명시한다. UCI가 `frr.conf`의 단일 소유자다.
  ...
  않는 것.
- 기존 파일은 지우지 않는다. 손으로 쓴 `frr.conf`가 이미 있으면 통째로
...
UI에서 OSPF를 켜면 uci에서 생성기, `frr.conf`를 거쳐 running-config에
...
주인은 누구인가"였다. 그 다음은 도구의 안전장치다. 생성기는 검증(드라이런)과
무변경 감지가 없으면 위험한 도구고, 스키마 기반 UI에는 반드시 탈출구가
필요하다. 탈출구가 없으면 사용자는 UI를 우회하고, 그 순간 소유권 계약이
깨진다. 그리고 "저장됨"은 "적용됨"이 아니고 "적용됨"은 "동작함"이 아니다.
확인은 항상 끝단에서 해야 한다.
