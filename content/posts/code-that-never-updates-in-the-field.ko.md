...
HDLC 프레임, 그중에서도 QCDM/DIAG 프레임이다. 앞부분이 깨져 보이는 이유도
같이 설명된다. 누군가 포트를 열고 자기 프로브 baud로 `tcsetattr` 한 뒤(115200이
...
그런데 영구 overlay에서 `rm`은 삭제가 아니라 upper에 남는 whiteout이다.
...
- tty는 화이트리스트로 한다. `ttyACM*`, `ttyUSB*`, `cdc-wdm*`. 우리가 파는
  모든 모뎀을 덮는다.
- net은 블랙리스트로 한다. 모뎀의 네트워크 인터페이스 이름은 예측할 수 없기
  때문이다(`wwan0`/`usb0`/…). 화이트리스트로 만들면 언젠가 WAN이 조용히 죽는다.
- no-bus 가드를 둔다. 모뎀은 S11에서 전원이 들어와 약 50초에 열거되는데 MM은
  S70이다. 그러니 대부분의 포트 이벤트는 MM이 없는 시점에 발생하고, 그때
  `mmcli`를 불러 봐야 6~13초를 쓰고 `couldn't get bus`로 실패할 뿐이다. 그래서
  이벤트만 캐시하고 포크를 건너뛴다. MM 래퍼가 시작할 때 캐시를 재생하므로
  정보는 잃지 않는다. 실제로 MM보다 약 90초 먼저 나타난 모뎀을 `mmcli -L`이
  정상 인식했다.
...
있는 문자열(`QUARK-MM-HOTPLUG-vN`)로 두고 앵커된 정확 일치로 검사한다.
