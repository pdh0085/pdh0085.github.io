...
carrier's questions is session logs and numbers, not "yes, supported."
...
## Option 43 — How a Blank Device Finds Its Management Server
...
### On by Default — But Latched After First Contact
...
seconds** and watched that discovery never reopened. And we checked that the
test wasn't spinning in place — that the daemon was actually alive and
cycling through retries — because a dead daemon produces the exact same
"nothing reopened" result while proving nothing.

We also chose the default's condition deliberately: not a "run once" flag,
but **"only when the ACS address is empty."** If discovery ever ran again on
a device already talking to an ACS, the next DHCP lease could swap out that
device's management server.
...
## The Measurement — 101.5 Seconds, 101 of Them Boot
...
value changed is not enough — we confirmed the radio actually restarted and
was beaconing that SSID, by scanning from a second device sitting next to
it. This project has produced several cases of "the status light is green
but nothing is actually passing."

## Remote Upgrade — and an Honesty Corner
...
the success report (TransferComplete, FaultCode 0). About **92 seconds**
from command to report. A corrupted image is bounced with an error in
**3.9 seconds**, without touching the flash.

Here is the honest part. **Images before a certain version never sent the
...
upgrade succeeded; the ACS just never found out — permanently. It's fixed
now, and when we demo on an older unit, we say so.
...
**HTTP.** A firewall pinhole is generated automatically, open only to the
...
**XMPP (TR-069 Annex K).** The device holds one outbound session, so zero
inbound ports are open. Call to session: **0.65 s** — but that number is
from a **test server**. Against a commercial ACS (GenieACS) we have
confirmed the session stays up across the internet with certificate
verification passing; the final step, an inbound call pushed from that side,
is still pending. On CGNAT (most 5G lines) this is the path that matters, so
we keep that distinction exact rather than blurring it.
...
forever — after which it quietly answers "no modem" defaults, permanently.
...
The fix replaces the permanent ban with a **30-second grace period and
retry**. Worst case, a 5-second delay every 30 seconds — in exchange, no
permanent loss. We drilled reproduce → fix → recover on real hardware, and
verified it **after a factory-clean flash**: the stronger check, proving the
fix is in the image, not just in the source tree.
...
- A silent fallback disguises an unverified feature as a working one. If
  flipping the switch does nothing behind the scenes, that fact must appear
  in a log.
- When you enable a convenience feature by default, design its exposure
  window with it. The security decision isn't on/off — it's *how long it
  stays open*. And you only claim it's latched after watching it stay
  latched for real time (394 seconds, in our case).
- Verify against the final effect, not the setting: the SSID actually in
  the air rather than the string in the UI; the image after a factory-clean
  flash rather than the source tree.
- Defensive logic that remembers failure forever becomes the failure. Grace
  and retry is almost always cheaper.
- Measurement conditions are part of the number. Writing "measured on
