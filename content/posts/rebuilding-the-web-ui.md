---
title: "Rebuilding the Web UI on LuCI (and Three Traps)"
...
That's the whole story about the screens. The real content of this post is
the traps we hit along the way.
...
- **stdin?** LuCI's `fs.exec` **has no stdin.** And the third argument looks
  like it might be stdin, but it is actually an **environment table**. Pass
  it a string and it gets **silently discarded** —
  `if (!L.isObject(env)) env = null`. That one kept me staring at "token is
  empty" errors for quite a while.
- **Environment variables?** The backend refuses them. rpcd rejects
  caller-supplied env on session (ACL-checked) calls
  (`if (sid && env)` → `PERMISSION_DENIED`). Which is the right security
  decision — otherwise a caller could change the behavior of a command the
  ACL had approved.
...
`form.Map` **re-renders itself on save.** Anything appended directly inside
it is gone on the next render.

The fix was structural: return the custom elements as **siblings of the
map** instead of appending into it. In a framework with declarative
rendering, anything you attach to the DOM imperatively lives exactly until
the next render.
...
- **Symptom**: the vnstat totals leapt by roughly 4 GiB on every
  reboot/reconnect.
- **The decisive clue**: one five-minute row in the database recorded
  `rx ≈ 4294965968` — almost exactly 2^32 — while the kernel counters for
  the same window showed a few hundred KB.
- **Root cause**: vnstat's `64bitInterfaceCounters` was at its default of
  `-2` (auto). The byte counters on `wwan0` reset to zero on every
  reboot/reconnect, and auto mode interprets that decrease as a **32-bit
  wraparound, adding 2^32 = 4.00 GiB.**
- **Fix**: force `64bitInterfaceCounters 1`. A 64-bit counter cannot wrap,
  so a decrease means a genuine reset — and nothing gets added.

And the fix had to land in **two places**: uci-defaults (new and
factory-reset boards) and a hotplug script (boards already deployed in the
field), because uci-defaults never runs again on a config-preserving
upgrade — the exact lesson from an earlier post in this series. One last
thing: history that was already polluted only goes away when the user resets
the usage statistics once. The config change only stops new inflation.
**A fix only repairs the future.**
...
We also found **two backends referenced by the UI that don't exist at
all** — `battery-mgrd` behind the battery widget and `xoleds` behind the LED
toggle. With no package present, they silently do nothing. A widget that
fails leaves no trace — which is exactly why it survives so long.
...
- DOM attached imperatively to a declarative UI lives for exactly one
  render.
- Counter bugs **accumulate**. Fixing the bug doesn't fix the past — the
