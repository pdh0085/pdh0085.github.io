...
This product has routing pages for RIP, OSPF, and BGP. Hit save and it
saves, no errors. But open `vtysh` and the routing daemons know nothing.
...
that FRR's startup script, `frrcommon.sh`, switches to integrated-config
mode the moment `/etc/frr/frr.conf` exists:
...
- Dry-run parse with `vtysh -C` before installing. A single syntax error
  takes down the entire routing stack on the next restart. If the config is
  rejected, the previous one keeps running and the page says so.
- Exit codes as a contract. `0` = new config installed (restart), `2` = no
  change (do NOT restart, because a pointless restart tears down
  adjacencies), `1` = rejected. Without a distinct "no change" outcome,
  every press of the save button shakes the routing.
- Keep an escape hatch. Any FRR syntax the schema can't model is emitted
  verbatim from per-section `raw` lists. FRR's configuration surface is
  wider than any schema; without this, a UCI frontend is a feature
  regression.
- Make ownership explicit. UCI is the sole owner of `frr.conf`. A single
  setting, `frr.global.managed='0'`, hands ownership back to a human, and
  the pages then show a "manual mode" banner. Don't pretend to be the owner
  when you're not.
...
`exec 1000>...; flock 1000`, and fd 1000 has no close-on-exec. The detached
process inherits that fd, and then watchfrr, mgmtd, zebra, and ospfd
inherit it in turn. The routing daemons hold our service's lock for as long
as FRR runs. The only fix is closing the fd at spawn time:
...
generator emits the per-interface `ip ospf passive` form instead. The
generated file and the running-config need to match character for
character, so the next person never has to wonder which one to trust.

## Lessons

The first question when building a configuration UI isn't "which fields do
we expose," it's "who owns this file." After that comes the tooling: a
generator without validation (a dry run) and no-change detection is a
dangerous tool, and a schema-based UI needs an escape hatch, because
without one users route around the UI and the ownership contract breaks
right there. And finally: "saved" is not "applied," and "applied" is not
"working." Always verify at the far end.
