...
아무것도 하지 않고 있었다. 고장 난 이유는 서로 달랐다 — 하나는 프로세스
경계, 하나는 추상화의 오작동.
...
`SAVE_CONFIG=1`일 때만 ubus 호출에 `backup`을 넣는다. 즉
`$UPGRADE_BACKUP`의 부재가 곧 초기화 신호다. 조건을 그걸로 바꾸고 시리얼로
양방향 확인했다 — `-n`이면 overlay 제거 로그가 찍히고 심어 둔 마커(APN,
호스트명)가 사라진다. 평소 업그레이드면 살아남는다.

교훈 한 줄. 셸의 `${VAR:-기본값}`은 편리하지만, "변수가 없다"와 "변수가
기본값이다"를 구분하지 않는다. 프로세스 경계를 넘는 코드에서는 위험하다.
...
1차 고장 — 볼륨을 못 찾는다. 순정 코드는 `volume_find("rootfs_data")`로
시작한다. 진짜 플래시 파티션이 있는 보드라면 당연한 가정인데, 우리 overlay는
FAT 위의 loop-ext4 이미지 파일이라 mtd(`/proc/mtd` 자체가 없다)·ubi·partname
어느 드라이버도 못 맞춘다.
...
1차 수정 — 그리고 그게 썩는다. "볼륨을 못 찾으면 마운트된 `/overlay`를
지운다"는 폴백을 넣었다. 잘 됐다. 루트가 squashfs loop로 바뀌기 전까지는.

2차 고장 — 추상화가 "성공"한다. 루트가 squashfs가 되자 `rootdisk`
...
영향이 없다. 덤으로 LuCI의 초기화 페이지도 고쳤다 — 종료 코드를 확인하지
...
- 복구 기능은 평소에 검증되지 않는다. 릴리스마다 실제로 눌러보는 수밖에
  없다. 이 두 버그 모두 정상 사용 경로에서는 영원히 보이지 않았을 것이다.
- "실패했는데 0을 반환한다"는 가장 나쁜 실패다. 초기화·업그레이드처럼
  결과를 눈으로 확인하기 어려운 기능일수록 종료 코드와 사후 상태를 함께
  확인해야 한다.
- 추상화는 모르는 걸 모른다고 말해줄 때만 안전망과 공존할 수 있다.
  `rootdisk`는 우리 배치를 이해하지 못하면서 자신 있게 답했고, 그 자신감이
  폴백을 죽였다. 물론 원인의 절반은 loop-ext4-on-FAT이라는 우리 쪽 특이한
  배치다 — 추상화가 상정한 세계 밖에서 살기로 했으면, 추상화의 답을
  의심하는 것도 우리 몫이다.
