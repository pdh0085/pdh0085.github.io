...
This final post is a retrospective on the way we worked rather than on the
results. The thing that consumed the most time in this project was not any
difficult bug. It was wrong certainty: incorrect facts that had been written
down as "already confirmed." A record like that narrows the search space of
the next person, who is usually yourself a few weeks later. Here are the
four beliefs this series disproved.

## Wrong Belief 1: "Verified by GPIO Readback"

This one is from the serial post. A readback only confirms that the intended
value landed in the register. It says nothing about what that value means to
the hardware. In reality the code had SER_MODE0 and SER_MODE2 swapped, so
...
moved was **peak boot RAM** (initmem freed went from 67456K to 1564K), while
steady-state `MemAvailable` stayed essentially unchanged at roughly
116–134 MB. Those 67 MB were being returned by the kernel during boot
anyway. The most common lie in optimization write-ups is not inventing
numbers. It is choosing the metric that looks improved.
...
with 0% ping loss. Failure records outlive success records, because nobody
ever tries again.

## Five Methods That Actually Worked

The four cases share one thing: progress happened only when the evidence
came from what the hardware was saying right now, not from documents,
comments, or memory.

- Read the answer off a working system. The ACPI RSDP address and the
  correct RS-232 GPIO values were both lifted straight from the old Yocto
...
- Verify at the far end. An interface holding an IP address is not proof of
  connectivity (the modem post), and a successful file write is not proof
...
- Log close to the hardware. The early-boot `/init` and the init-script
  profiler both write to `/dev/kmsg`, so with a serial console you see
  everything even when networking is dead.

## How We Write Documents Now

- Every fact carries how we know it. Not "verified" but "confirmed on a real
  board with this command." The rule goes down to a single connector name (a
  document said CON8; it was actually CON6).
...
Leaving the unfinished list in plain sight is part of the record too. The
thing I did most often on this project was not fixing code. It was taking
something that someone, usually me, had written down as already confirmed,
and confirming it again in front of the hardware.
