...
The one fact we allow ourselves to depend on is this: **the modem is always
...
### The Problem Wasn't a Symptom — It Was the Absence of One
...
The result: `cdc-wdm0` was never reported to MM → MM marked the interface
`wwan0 (ignored)` → demotion to AT+PPP. Not one line of error output.
...
Same modem, different manufacturer string. **Where the string comes from
tells you which path you're on.** Ask over QMI and the chipset vendor
answers; ask over AT and the module vendor answers.
...
One more thing: this bug is **invisible on the PLS83**, because that module
has no cdc-wdm to lose. It only shows up with a Quectel module fitted — a
bug that requires a hardware configuration change to become observable. A
...
For this SIM, APN auto-profiling **silently half-fails.** The modem
attaches, but to a bearer that passes no data. Setting the carrier's correct
APN explicitly made it immediately reattach to a real bearer (`10.x/23`),
and ping, HTTP, and HTTPS all worked.
...
- In a cellular stack, the only trustworthy signal is **end-to-end
  traffic.** `connected`, an IP address, a default route — we confirmed
  twice that all of them can be false positives.
- A product that supports multiple modules will inevitably grow **bugs that
  only one module can reveal.** Code should depend on invariants — here, the
  USB port position `1-1` — not on models.
- Fallbacks look like kindness, but **a silent fallback hides bugs.** If the
  ...
  sooner.
- It pays to read diagnostic output for **where a value comes from, not just
  what it says.** Which protocol answered the `manufacturer` query is what
  told us which path we were on.
