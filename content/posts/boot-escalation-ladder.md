---
title: "The Boot Escalation Ladder: Designing for Failure"
...
**state machine**. Where the state lives, what consumes it, and what does
*not* consume it — that turned out to be the entire design.
...
heroes. But golden can do **nothing at all** in the two situations that
matter most:
...
To be honest about how it was found: **this rung came out of a design review,
not a test.** I had the ladder drawn out as a table and traced "where does a
factory-fresh board go if its first boot hangs?" — and there was the hole. It
was a bug caught on paper, not reported by hardware, which made it scarier:
the first place it would have been stepped on was the field.
...
- The substitution placeholder had to appear **exactly once** — a global
...
## How We Knew It Could Be Trusted: The Power-Pull Test
...
- A fallback is a state machine, and the central design question is **what
  consumes the state**. Only a hang consumes the marker; a failed validation
  skips without consuming it — that one distinction holds up the whole
  ladder.
- A safety net that isn't regularly refreshed rots. Golden = the latest
...
- Design review catches a class of bugs that testing can't. The most
  ...
  board.
- A failure path is not validated **until you have made it fail on
  purpose**. Pulling the plug took one second, and that second turned this
  design from a story into a fact.
